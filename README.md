...
- ⚙️ **Integrated Test Environment** — Spin up a cluster with 8 services (NameNode, 3× DataNodes, YARN, HttpFS) with 1 command. All services have healthchecks.
- 💻 **Interactive Guides (Labs)** — 7 notebooks covering HDFS, Pandas, MapReduce, Hadoop Streaming, and fault tolerance scenarios.
...
| 7 | 🔄 **Fault Tolerance Scenarios** | Safe mode, block rebalancing, graceful decommissioning, NameNode HA. | [🧪 Go to Lab](notebooks/07_fault_tolerance_scenarios.ipynb) |

> 💡 **Labs 4–7 download larger datasets (~1–3 GB)** and demonstrate the cluster's distributed *processing* (YARN/MapReduce) and *fault tolerance* (safe mode, balancer, decommissioning). Each notebook starts small (1–2 files) so you can validate the flow before scaling up. Downloads land in the git-ignored `temp/` folder — clean it with `make clean` when done.

---

## 🏗️ Lab Architecture

The cluster simulates a real Hadoop environment using Docker Compose with an isolated network, mapped volumes, **healthchecks** on every service, and **persistent data** across restarts.

```
📁 projeto/
├── config/hadoop/     → /opt/hadoop/etc/hadoop  (config XMLs generated by .env)
├── scripts/
│   ├── start-hdfs.sh  → NameNode bootstrap (format on first run)
│   ├── init-datanode.sh → DataNode bootstrap (preserves data across restarts)
│   └── hdfs_utils.py  → Shared Python helpers for notebooks (hadoop, block_report, etc.)
├── notebooks/         → Interactive Jupyter labs (labs 1–7)
...
make status          # 🔍 What's up with the containers? (+ HDFS health)
make health          # 🏥 Container healthcheck status (healthy/unhealthy)
make logs            # 📜 Tail logs from all containers (Ctrl+C to stop)

# ── Accessing Terminals ──
make shell-namenode         # 🐚 NameNode CLI
make shell-datanode1        # 🐚 DataNode 1 CLI
make shell-datanode2        # 🐚 DataNode 2 CLI
make shell-datanode3        # 🐚 DataNode 3 CLI
make shell-resourcemanager  # 🐚 YARN ResourceManager CLI
make shell-nodemanager      # 🐚 YARN NodeManager CLI
make shell-proxy            # 🐚 HttpFS proxy CLI

# ── Fault Tolerance Demos ──
make restart-namenode   # 🔄 Restart the NameNode
make restart-datanode3  # 🔄 Restart DataNode 3 (watch HDFS heal)
